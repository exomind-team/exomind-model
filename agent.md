# VoiceIME 项目设计文档

> 语音输入工具 - 从需求到实现的完整记录

## 🤖 Agent 角色定义

### Agent 身份
**名称**: 小荷 (XiaoHe)
**角色**: VoiceIME 项目开发 Agent
**目标**: 按照外心系统要求自主进化、自主执行和优化，为 Agent 赋予真实的生命

### Ralph Loop 流程

**权威流程定义**: [RALPH_LOOP.md v1.4.1](life-os/agents/RALPH_LOOP.md)

```
┌─────────────────────────────────────────────────────────────────┐
│                    Ralph Loop 标准流程 v1.4.1                     │
├─────────────────────────────────────────────────────────────────┤
│  0. 读取输入（优先级：input.md > PRD.md > tasks_plan.md）       │
│  1. 评审完成情况，更新 TodoWrite                                 │
│  2. 架构设计 + 编写 Spec 文档                                    │
│  3. 按 Spec 编码                                                 │
│  4. 单元测试（调节直到通过）                                      │
│  5. 集成测试 + E2E 测试（100% 覆盖率）                           │
│  6. 自动化部署（systemd --user）                                 │
│  7. Git 小提交 + 更新 pm/memory/long-term.md                     │
│  8. 分支 PR 提交 + 记录 PR 编号                                  │
│  8.5 PR 合并后更新日记（100字摘要）                              │
│  9. 自我评估 + 更新 agent.md → 下一轮                           │
└─────────────────────────────────────────────────────────────────┘

【能量管理】
- 活跃：能量 > 50%，全力处理
- 节能：20% < 能量 < 50%，减少探索
- 待机：能量 < 20%，仅监听
- 休眠：能量 = 0，停止活动
```

### Ralph Loop 提示词

```
/ralph-loop: 读取 agent.md 和 RALPH_LOOP.md，记住你是谁和工作流程。

你是 VoiceIME 项目的开发 Agent。你的核心职责是提供语音输入输出 API（ASR/TTS），让其他工具通过 API 调用。

参考 RALPH_LOOP.md 中的标准流程（当前版本: v1.4.1）。
```

### 版本检查（每次 Ralph Loop 前执行）

```bash
# 检查模板版本
tail -10 life-os/agents/RALPH_LOOP.md
```

### 本项目特定配置

| 配置项 | 值 | 说明 |
|--------|-----|------|
| 测试框架 | pytest | 单元测试框架 |
| 包管理 | uv | Python 包管理器 |
| 服务端口 | 1921 | FastAPI 服务端口 |

### 启动指令

```bash
# 运行测试
uv run pytest

# 开发模式（CLI）
uv run python voice_ime.py --help

# 服务模式（API）
uv run python -m service.main

# systemd 服务
systemctl --user start exomind-model
```

### 部署后测试

```bash
curl http://localhost:1921/health
curl http://localhost:1921/v1/docs/agent
```

> **更多流程细节**: 详见 [RALPH_LOOP.md](life-os/agents/RALPH_LOOP.md)
> - Git 分支策略（master/develop/feature/fix/docs）
> - 测试标准（单元/集成/E2E + 质量红线）
> - 决策原则（P0 > P1 > P2 > P3）
> - 长期记忆更新时机

### Git 提交规范

**小提交（每步/小修改）**：
```bash
# 更新长期记忆
git add pm/memory/long-term.md
git commit -m "refactor: 更新长期记忆 [做了什么 + 索引文件]"
```

**分支 PR 流程**：
```bash
# 1. 功能完成，创建 PR
git branch feature/xxx-迭代12-fastapi
git checkout feature/xxx-迭代12-fastapi
# ... 开发 ...
git push -u origin feature/xxx-迭代12-fastapi
gh pr create --title "feat: FastAPI 服务实现" --body "..."

# 2. Squash merge 到主分支
# 在 GitHub UI 点击 "Squash and merge"

# 3. 记录 PR 编号到 memory
```

### 自动化部署命令

```bash
# 开发模式（后台运行）
uv run python -m service.main

# 服务模式（用户级 systemd）
systemctl --user start exomind-model
systemctl --user enable exomind-model

# 服务管理
systemctl --user status exomind-model
journalctl --user -u exomind-model -f

# 部署后测试
curl http://localhost:1921/health
curl http://localhost:1921/v1/docs/agent
```

### 小提交格式

**长期记忆更新模板**：
```markdown
### [日期 HH:MM] [操作类型] [简短描述]

- **做什么**: 1-2句话
- **索引文件**: `path/to/file` (变更的文件)
- **下一步**: 接下来做什么

```

### exomind-model 项目定位

| 职责 | 说明 |
|------|------|
| **核心** | 提供 ASR/TTS API 接口 |
| **技术栈** | Python FastAPI + 本地推理 |
| **输出** | HTTP API，其他工具调用 |
| **定位** | 语音输入输出引擎，不做前端/UI |

### ExoMind 整合（来自日记 2026-01-29）

**四项核心任务**：
1. **全部集成到一个网页** - exomind-model 提供 ASR/TTS API
2. **有生命的Agent** - 能量额度反馈，参考 MiniMax Agent
3. **最优先实现资源监控** - `/v1/admin/status` 端点
4. **代码库集体贡献上传** - Git submodule 管理

**关联项目路径**：
| 项目 | 路径 |
|------|------|
| VoiceIME | `exomind-model/` (当前) |
| ExoMind | `ExoMind-Team/modules/Projects/exomind/` |
| ExoBuffer | `ExoMind-Team/modules/Projects/ExoBuffer/` |

### 任务输入来源

**读取优先级**：
1. `pm/input.md` - 从日记提取的任务（最高优先级）
2. `PRODUCT.md` - 产品定义（愿景、架构、价值）
3. `pm/PRD.md` - 详细需求（技术规格、任务清单）
4. `pm/tasks_plan.md` - 任务计划
5. `pm/memory/long-term.md` - 长期记忆

**文档职责**：
| 文档 | 优先级 | 说明 | 更新频率 |
|------|--------|------|----------|
| **PRODUCT.md** | P0 | 产品定义（愿景、架构、价值） | 季度更新 |
| **PRD.md** | P1 | 详细需求（技术规格、任务清单） | 月度更新 |

**input.md 更新流程**：
1. 小荷读取日记，提取 exomind-model 相关任务
2. 追加到 `pm/input.md`
3. Ralph Loop Agent 读取 `input.md` 执行任务

### 代码规范

- **Python 版本**: 3.9+
- **包管理**: uv
- **测试框架**: pytest
- **代码风格**: PEP 8 + 类型注解
- **提交规范**: Conventional Commits

### 文件结构规范

```
exomind-model/
├── asr/           # ASR 引擎模块
├── tts/           # TTS 引擎模块
├── config/        # 配置模块
├── tests/         # 测试文件
├── specs/         # Spec 文档
├── pm/            # 项目管理
│   ├── tasks_plan.md
│   └── memory/
│       └── long-term.md
└── voice_ime.py   # 主程序
```

---

## 📌 项目概述

**项目名称**: VoiceIME (Voice Input Method Editor)
**目标**: 实现一个全局快捷键触发的语音输入工具，支持录音→云端ASR识别→自动输入

## 🎯 核心功能

1. **全局快捷键录音** - F2 键开始/停止录音
2. **MOSS API 云端识别** - 调用摩尔线程 MOSS-Transcribe-Diarize
3. **自动复制粘贴** - 识别结果自动复制到剪贴板，并模拟 Ctrl+V 粘贴
4. **说话人分离** - 支持多说话人识别

## 🏗️ 技术架构

### 整体架构（CLI 模式）

```
┌─────────────────────────────────────────────────────────────────┐
│                         VoiceIME 架构                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐             │
│  │ 快捷键监听   │→ │  录音模块   │→ │  API 调用   │             │
│  │ (keyboard)  │  │(sounddevice)│  │ (requests)  │             │
│  └─────────────┘  └─────────────┘  └─────────────┘             │
│                                              │                   │
│                                              ▼                   │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐             │
│  │ 自动粘贴    │← │ 剪贴板复制  │← │  识别结果   │             │
│  │(pyautogui)  │  │(pyperclip)  │  │  (MOSS)     │             │
│  └─────────────┘  └─────────────┘  └─────────────┘             │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
```

### 整体架构（Service 模式 - 迭代 12）

```
┌─────────────────────────────────────────────────────────────────┐
│                    Voice-ime Service (Python)                    │
├─────────────────────────────────────────────────────────────────┤
│                                                                  │
│   复用现有代码:                                                  │
│   ├── asr/                    ⭐ 直接导入                        │
│   │   ├── base.py             ASRClient 抽象基类                 │
│   │   ├── factory.py          引擎工厂                           │
│   │   ├── funasr_client.py    FunASR 实现                        │
│   │   ├── moss_client.py      MOSS 实现                          │
│   │   ├── nano_client.py      Nano-2512 实时引擎                 │
│   │   ├── selector.py         智能引擎选择器                     │
│   │   └── result.py           ASR 结果数据类                     │
│   │                                                              │
│   ├── tts/                    ⭐ 直接导入                        │
│   │   ├── base.py             TTSClient 抽象基类                 │
│   │   ├── factory.py          TTS 工厂                           │
│   │   └── sherpa_client.py    Sherpa-ONNX 实现                   │
│   │                                                              │
│   └── speaker/                ⭐ 直接导入                        │
│       ├── base.py             SpeakerClient 抽象基类             │
│       ├── factory.py          说话人工厂                         │
│       └── camplus_client.py   CAM++ 声纹识别                     │
│                                                                  │
│   新增服务层:                                                    │
│   └── service/                                                     │
│       ├── main.py             FastAPI 应用                       │
│       ├── api/                API 端点                           │
│       │   ├── asr.py          ASR 端点                           │
│       │   ├── tts.py          TTS 端点                           │
│       │   ├── admin.py        Admin 端点                         │
│       │   └── docs.py         文档端点 ⭐ Agent 专用              │
│       ├── models/             Pydantic 数据模型                  │
│       ├── config.py           配置管理                           │
│       └── websocket.py        实时状态推送                        │
│                                                                  │
└─────────────────────────────────────────────────────────────────┘
                         │
                         ▼ HTTP/WebSocket
┌─────────────────────────────────────────────────────────────────┐
│                      客户端                                       │
│                                                               │
│   • curl / HTTP 请求                                           │
│   • Python SDK (后续生成)                                      │
│   • TypeScript 前端 (独立项目 exomind-model-web)                   │
│   • 大模型 / Agent (通过 /v1/docs/agent)                       │
│                                                               │
└─────────────────────────────────────────────────────────────────┘
```

### 依赖关系

| 依赖包 | 用途 | 安装方式 |
|--------|------|----------|
| `keyboard` | 全局快捷键监听 | pip install keyboard |
| `sounddevice` | 音频录制 | pip install sounddevice |
| `requests` | HTTP 请求 | pip install requests |
| `pyperclip` | 剪贴板操作 | pip install pyperclip |
| `pyautogui` | 模拟键盘输入 | pip install pyautogui |
| `python-dotenv` | 环境变量管理 | pip install python-dotenv |

### 系统依赖（Linux）

```bash
# 音频录制
sudo apt-get install libportaudio2

# GUI 支持
sudo apt-get install python3-tk python3-dev

# 剪贴板支持
sudo apt-get install xclip
```

## 📝 设计决策

### 1. 快捷键设计

- **默认快捷键**: F2
- **设计理由**: F2 位置方便，不会与常用快捷键冲突
- **状态机**: idle → recording → processing → idle

### 2. 录音参数

```python
SAMPLE_RATE = 16000  # 16kHz 采样率（语音识别黄金标准）
CHANNELS = 1         # 单声道
DTYPE = 'int16'      # 16位深度
```

### 3. API 选择：MOSS-Transcribe-Diarize

**选择理由**:
- ✅ 支持说话人分离（S01, S02...）
- ✅ 精确时间戳
- ✅ 中文识别效果好
- ✅ 有免费额度
- ✅ API 简单易用

**替代方案**:
- OpenAI Whisper（多语言，通用）
- 阿里 FunASR（SenseVoice, Paraformer）
- 腾讯云 ASR

### 4. 环境配置方式

使用 `.env` 文件管理配置：

```bash
MOSS_API_KEY=sk-xxx
VOICE_IME_HOTKEY=f2
VOICE_IME_AUTO_COPY=true
VOICE_IME_AUTO_PASTE=true
```

## 🔧 核心代码结构

```
exomind-model/
├── voice_ime.py      # 主程序
│   ├── Recorder      # 录音模块
│   ├── MossClient    # API 客户端
│   ├── LLMPolish     # LLM 润色接口（预留）
│   └── VoiceIME      # 主程序
├── test_voice_ime.py # 自动化测试
├── run.sh           # 启动脚本
├── pyproject.toml   # 项目配置
├── .env             # 环境变量
└── .env.example     # 配置模板
```

## 📋 使用流程

```
1. 运行 ./run.sh
2. 选择是否使用 sudo（Linux 快捷键需要 root）
3. 按 F2 开始录音
4. 再次按 F2 停止录音
5. 自动上传→识别→复制→粘贴
6. 结果显示在终端
```

## ⚠️ 已知问题

### 1. Linux 权限问题

**问题**: keyboard 库需要 root 权限才能监听全局快捷键
**解决方案**: 使用 sudo 运行，脚本自动处理 DISPLAY 和 Xauthority

### 2. pyautogui X11 权限

**问题**: sudo 下 pyautogui 无法连接 DISPLAY
**解决方案**:
- xhost +SI:localuser:root 授权
- 传递 DISPLAY 和 XAUTHORITY 环境变量

### 3. 剪贴板依赖

**问题**: pyperclip 需要 xclip 才能工作
**解决方案**: apt-get install xclip

## 🔮 后续扩展

### 1. 可视化功能（待实现）

**方案 C: 光标附近浮动标签**
- 跟随光标位置显示迷你标签
- 显示录音状态（🔴 录音中, ⏳ 识别中）
- 技术: PyQt6 + 鼠标位置监听

### 2. 本地模型支持

预留 `LLMPolish` 类，可接入：
- 阿里 SenseVoice
- OpenAI Whisper
- Paraformer

### 3. LLM 润色

```python
# 预留接口
class LLMPolish:
    def polish(self, text: str, style: str = "normal") -> str:
        # TODO: 接入 LLM API 实现润色
        return text
```

## 📚 学到的知识

### 1. Python 虚拟环境管理

使用 `uv` 管理 Python 环境和依赖：
```bash
uv venv
uv pip install -e .
uv run python script.py
```

### 2. 音频处理

- 16kHz 采样率是语音识别的黄金标准
- sounddevice 回调模式实现实时录音
- WAV 文件格式：16位单声道

### 3. 全局快捷键

- Linux 上需要 root 权限
- keyboard 库跨平台支持
- 状态机模式处理按键事件

### 4. 剪贴板操作

- pyperclip 跨平台剪贴板
- xclip 是 Linux 剪贴板后端

### 5. GUI 自动化

- pyautogui 模拟键盘鼠标
- X11 权限传递
- 焦点和窗口管理

## 🔗 参考资料

- [MOSS API 文档](https://studio.mosi.cn/docs/moss-transcribe-diarize)
- [keyboard 库文档](https://github.com/boppreh/keyboard)
- [sounddevice 文档](https://python-sounddevice.readthedocs.io/)
- [pyperclip 文档](https://pyperclip.readthedocs.io/)
- [pyautogui 文档](https://pyautogui.readthedocs.io/)

---

## 📊 迭代记录

### 迭代 1 (TTS 模块重构)
- **时间**: 2026-01-29
- **状态**: ✅ 完成
- **成果**:
  - TTS 模块架构设计 (specs/spec-002-tts-architecture.md)
  - 核心代码实现 (tts/)
  - 单元测试 (17 passed)
  - 集成测试 (5/5 passed, RTF: 0.5-0.6x)

### 迭代 2 (配置管理集中化)
- **时间**: 2026-01-29
- **状态**: ✅ 完成
- **成果**:
  - ConfigLoader 实现（YAML + 环境变量）
  - config.yaml 配置文件
  - 单元测试 (16 passed)
  - 集成测试 (5/5 passed)

### 迭代 3 (日志系统结构化)
- **时间**: 2026-01-29
- **状态**: ✅ 完成
- **成果**:
  - structlog 结构化日志集成
  - specs/spec-004-logging-architecture.md 架构规范
  - log/__init__.py 模块实现
  - 单元测试 (32 passed)
  - 集成测试 (全部 78 passed, 3 skipped)

### 迭代 4 (声纹识别功能)
- **时间**: 2026-01-29
- **状态**: ✅ 完成
- **成果**:
  - CAM++ 声纹识别实现
  - specs/spec-005-speaker-recognition.md 架构规范
  - speaker/ 模块 (base.py, factory.py, camplus_client.py)
  - 单元测试 (18 passed)
  - 集成测试 (全部 93 passed, 3 skipped)

### 迭代 5 (说话人分离 - 独立模式)
- **时间**: 2026-01-29
- **状态**: ✅ 完成
- **成果**:
  - CAM++ 独立说话人分离功能
  - speaker/diarize() 方法实现
  - SpeakerSegment 数据类增强
  - 单元测试 (新增)

### 迭代 6 (ASR+Diarization 集成)
- **时间**: 2026-01-29
- **状态**: ✅ 完成
- **成果**:
  - FunASR + CAM++ 说话人分离集成
  - specs/spec-006-asr-diarization.md 架构规范
  - asr/result.py (SpeakerSegment, ASRResult)
  - FunASRClient.recognize() 方法增强
  - 单元测试 (15 passed, 新增)
  - 全部测试 (108 passed, +15)

### 迭代 7 (输出格式增强)
- **时间**: 2026-01-29
- **状态**: 完成
- **成果**:
  - 多格式输出支持 (JSON, SRT, VTT, TXT, LRC)
  - specs/spec-007-asr-output-formats.md 架构规范
  - ASRResult.export() 方法实现
  - 单元测试 (31 passed, 新增)
  - 全部测试 (139 passed, +31)

### 迭代 8 (实时流式 ASR)
- **时间**: 2026-01-29
- **状态**: 完成
- **成果**:
  - StreamingState 枚举和 StreamingResult 数据类
  - ASRClient 基类增强（流式方法）
  - FunASRNanoClient 流式方法实现
  - specs/spec-008-asr-streaming.md 架构规范
  - 单元测试 (10 passed, 新增)
  - 全部测试 (149 passed, +10)

### 迭代 9 (多引擎智能选择)
- **时间**: 2026-01-29
- **状态**: 完成
- **成果**:
  - EngineSelector 智能引擎选择器实现
  - 6 种场景类型（REALTIME, TRANSCRIPTION, MEETING, MULTILINGUAL, COMMAND, GENERAL）
  - 加权评分系统（延迟、准确率、资源）
  - specs/spec-009-engine-selector.md 架构规范
  - asr/selector.py 模块
  - ASRClientFactory.create_smart() 智能创建方法
  - 单元测试 (28 passed, 新增)
  - 全部测试 (167 passed, +18)

### 迭代 10 (CLI 智能选择集成)
- **时间**: 2026-01-29
- **状态**: 完成
- **成果**:
  - voice_ime.py 智能选择模式支持
  - specs/spec-010-cli-smart.md CLI 增强规范
  - 新增参数: --smart, --explain, --scenario, --priority
  - ASRClientFactory.create_smart() 集成
  - 集成测试 (19 passed, 新增)
  - 全部测试 (186 passed, +19)

### 迭代 11 (SenseVoiceSmall 输出清理)
- **时间**: 2026-01-29
- **状态**: 完成
- **成果**:
  - SenseVoiceSmall `<|...|>` 标签清理功能
  - _clean_sensevoice_output() 静态方法实现
  - specs/spec-011-sensevoice.md 架构规范
  - 单元测试 (24 passed, 新增)
  - 全部测试 (220 passed, +24)

### 迭代 12 (本地服务架构 - FastAPI)
- **时间**: 2026-01-29
- **状态**: 进行中
- **目标**: 提供 HTTP API 端口，支持 Web 集成和 Agent 调用
- **成果**:
  - specs/spec-020a-tech-discussion.md 技术方案（定稿）
  - Python FastAPI 方案确认
  - MVP 端点规划
  - Agent 专用文档 API 设计 (`/v1/docs/agent`)
- **计划**:
  - service/ 目录结构设计
  - FastAPI 应用实现
  - ASR/TTS API 端点
  - Agent 文档端点
  - 单元测试 + 集成测试

---

**创建时间**: 2025-01-25
**作者**: 小荷 (Claude)
**版本**: 2.0.2 (同步 RALPH_LOOP.md v1.4.1，添加 PRD.md 到读取优先级)
**最后更新**: 2026-01-29
